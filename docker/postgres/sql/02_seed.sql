-- =============================================================================
-- Datos de ejemplo para PostgreSQL, gemelos de docker/oracle/sql/02_seed.sql.
--
-- Las fechas se calculan relativas a NOW() para que el seed siga teniendo sentido
-- sin importar cuándo se levante el contenedor: si se fijaran literales, las
-- citas nacerían caducadas.
--
-- Se asume que 01_schema.sql acaba de recrear las tablas, así que las secuencias
-- IDENTITY arrancan en 1 y las FK del bloque de citas pueden ir por literal.
-- =============================================================================
...
INSERT INTO APPOINTMENTS (FK_BRANCH, FK_CLIENT, GUEST_NAME, SCHEDULED_AT, DURATION_MIN, STATUS, DETAILS) VALUES
  (1, 1, NULL, NOW() + INTERVAL '1 day', 45, 'CONFIRMED', 'Revisión general y cotización.'),
  (2, 2, NULL, NOW() + INTERVAL '2 day', 30, 'PENDING',   'Seguimiento del contrato 8891.');
...
INSERT INTO APPOINTMENTS (FK_BRANCH, FK_CLIENT, GUEST_NAME, SCHEDULED_AT, DURATION_MIN, STATUS, DETAILS) VALUES
  (1, NULL, 'Carlos Méndez (walk-in)', NOW() + INTERVAL '3 hour', 20, 'PENDING', 'Llegó sin cita, pidió informes.'),
  (3, NULL, 'Prospecto telefónico',    NOW() + INTERVAL '5 day',  60, 'PENDING', 'Cita agendada por teléfono, aún sin alta como cliente.');
